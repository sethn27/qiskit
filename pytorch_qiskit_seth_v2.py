# Using Sampler QNN, 1 CNN, attempting PPO loss
...
class HybridActorNetwork(nn.Module):
    ...
    def forward(self, current_state):
        ...
        output_layer_1 = 1*output_layer_1[0] + (-1)*output_layer_1[1] #Calculating expectation
...
class QERL_Agent():
    ...
    def loss_function(self,
        ...
        actor_predict = [] 
        for i in range(self.log_policies.shape[0]):  
            a, b  = self.action_selection(self.states[i])
            actor_predict.append(b.item())
        actor_predict = torch.Tensor(actor_predict)    
        
        prob_ratio = actor_predict.exp() / self.log_policies.exp()
        weighted_probs = advantage.detach() * prob_ratio
        weighted_clipped_probs = torch.clamp(prob_ratio, 1 -0.02, 1+0.02)*advantage.detach()
        
        actor_loss = -torch.min(weighted_probs, weighted_clipped_probs).mean() #Same eqn as classical PPO
        
        critic_loss = torch.mean((discounted_rewards - self.state_values)**2)
    ...
    def training(self, next_state_value):
        ...
        #for t in range(self.number_of_epochs): # EPOCHS HERE??
...
max_time_steps = 500
number_of_epochs = 4 #Set but not used, due to error
